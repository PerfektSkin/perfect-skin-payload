...
export const generateOffersMeta = async (doc: Offer | null): Promise<Metadata> => {
  ...
  const offerImage = doc?.image && typeof doc.image === 'object' ? doc.image : null
  const ogImage = getImageUrl(offerImage) || getImageUrl(siteSettings?.ogImage as Media | null)
  ...
  return {
    ...
    openGraph: mergeOpenGraph({
      ...
      title,
      url: doc?.slug ? `/offers/${doc.slug}` : '/offers',
    }),
    title,
  }
}
